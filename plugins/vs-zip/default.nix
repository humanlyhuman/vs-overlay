...
let
  mkZigDep = name: src: runCommand name {} ''
    mkdir -p $out
    cd ${src}
  ...
  deps = linkFarm "vszip-zig-deps" [
    (let name = "vapoursynth-4.0.0-jLYMQ799AgCA8sL5lgewK9acIrAKjs-ByT2pdKI5dHq2"; in {
      inherit name;
    ...
    (let name = "zigimg-0.1.0-8_eo2jNrFQD4mu3EAUkfQRmCkyfprdIXc8JQ6uyxhjSQ"; in {
      inherit name;
      path = mkZigDep name (fetchgit {
